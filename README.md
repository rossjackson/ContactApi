# ContactApi
ContactApi is a sample of a REST service that showcases a production quality application.  It includes unit testing, ...
...
### Folder Structure
**lib**: This is where I set Nuget to store all the third party binaries I used for ContactApi.  It includes but not limited to log4net, Moq, Ninject, etc.
**logs**: This is where I set log4net to store all the exception that happens within the ContactApi solution.
**src**: This is where the ContactApi.sln resides and all the projects related to it.

### Projects
**ContactApi.Data**: Contains ContactApi's model plain old CLR objects, ContactApiDb context, and the ContactService.  I used entity framework for my sql database mapper to be translated to its corresponding POCO.
...
**Tests/ContactApi.Web.Api.Tests**: Unit tests for the controllers and models.  I tested the different data annotations for the models.  It is unnecessary but I want to showcase how you can test the ModelState (data annotations).
...
In object oriented programming, a major philosophy of design is loose coupling.  Dependency injections make this thing happen.  You want to be able to use classes that aren't tightly coupled to one another.  It should use interfaces than concrete classes.
...
The Kernel creation calls the `NinjectConfigurator` which where I bind all the the necessary DI.  For this example I have the `log4net` and the `IContactService`.

### Unit Testing
ContactApi uses [Moq](https://github.com/Moq/moq4/wiki/Quickstart) and [NUnit](https://nunit.org/) for unit testing.  Unit testing is now implemented by a lot of software companies.  A code without tests is a bad code.  With tests, we can change our code quickly and verifiably.  I created 2 major test projects.  `ContactApi.Data.Tests` tests the very heart of the application which is the `ContactServices` and the other project is the `ContactApi.Web.Api.Tests` which tests the controllers and model's data annotataions.  As stated earlier, the data annotations / modelstate doesn't need to be tested but I want to show how it is done if someone insists.
...
I ignored this files for security purposes.
